/*
Turn a set of behaviours into objects and make them interop inside
original context object.

Context holds reference to a strategy object. It delegates executing the
behavuoour to the linked strategy object. In order to change the way the context
performs its work, other objects may replace the currently linked strategy object
with another one.
*/
