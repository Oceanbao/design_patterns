...
// Custom Iterator for traversal through UserCollection abstracted
pub struct UserIterator<'a> {
    index: usize,
    user_collection: &'a UserCollection,
}

// `Iterator` is std interface for dealing with iterators from stdlib
...
fn main() {
    ...
    let mut user_iterator = users.iter();

    print!("1st element: {:?}", user_iterator.next());
